document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateUserProfileUI(name, sign, city) {
    ...
    if (profileSign)
      profileSign.innerHTML = `<span style="font-size: 1.5rem; vertical-align: middle;">${symbol}</span> ${sign}`;
    ...
    if (chartSummary) {
      ...
      const description =
        element === "fire"
          ? "bold passion and leadership 🦁"
          : element === "water"
            ? "deep emotion and intuition 🌊"
            : element === "earth"
              ? "stability and practical wisdom 🏔️"
              : "intellect and social curiosity 🦋";
    ...
    if (cosmicVibe) {
      cosmicVibe.innerText = `You radiate ${
        element === "fire"
          ? "bold, dynamic"
          : element === "water"
            ? "deep, intuitive"
  ...
  function openDetail(profile) {
    ...
    document.getElementById("detailSignInfo").innerText =
      `${signSymbol} ${profile.sign} • ${element}`;
    ...
    hobbiesContainer.innerHTML = profile.hobbies
      .map((h) => `<span>${h}</span>`)
      .join("");
  ...
  function renderMatchesList() {
    ...
    list.innerHTML = matchedProfiles
      .map((p, index) => {
  ...
  function initDrag() {
    ...
    const onStart = (e) => {
      ...
      e.preventDefault(); // important on touchpads
    };
    ...
    const onMove = (e) => {
      ...
      if (Math.abs(currentX) > 12) {
        hasMoved = true;
      }

      card.style.transform = `translateX(${currentX}px) rotate(${currentX / 9}deg) scale(${1 - Math.abs(currentX) / 1000})`;
    };
    ...
    const onEnd = () => {
      ...
      // Give click handler time to see final state
      setTimeout(() => {
        currentX = 0;
        hasMoved = false;
      }, 80);
    ...
    card.addEventListener("click", (e) => {
      if (!hasMoved && Math.abs(currentX) < 15) {
  ...
  const profiles = [
    {
      ...
      age: 24,
      emoji: "🦁",
      vibe: "He radiates bold, dynamic fire energy.",
      bio: "Always finding the spotlight...",
      hobbies: ["Hiking", "Tech"],
      details: "Sun in Leo, Moon in Aries",
    },
    {
      name: "Lyra",
      sign: "Libra",
      age: 22,
      emoji: "⚖️",
      vibe: "She exudes harmony and curious air energy.",
      bio: "Searching for balance...",
      hobbies: ["Yoga", "Art"],
      details: "Sun in Libra, Moon in Taurus",
    },
    {
      name: "Orion",
      sign: "Scorpio",
      age: 26,
      emoji: "🦂",
      vibe: "He channels deep, intuitive water energy.",
      bio: "Still waters run deep...",
      hobbies: ["Chess", "Nightwalks"],
      details: "Sun in Scorpio, Moon in Pisces",
    },
    {
      name: "Luna",
      sign: "Cancer",
      age: 23,
      emoji: "🦀",
      vibe: "She flows with gentle, nurturing water energy.",
      bio: "Home is where the heart is...",
      hobbies: ["Cooking", "Poetry"],
      details: "Sun in Cancer, Moon in Scorpio",
    },
    {
      name: "Nova",
      sign: "Aquarius",
      age: 21,
      emoji: "🏺",
      vibe: "She vibrates with visionary air energy.",
      bio: "Dreaming of the future...",
      hobbies: ["Coding", "Sci-Fi"],
      details: "Sun in Aquarius, Moon in Gemini",
    },
  ];
  ...
  function getCompatibility(s1, s2) {
    ...
    if ((e1 === "fire" && e2 === "air") || (e1 === "air" && e2 === "fire"))
      return 88; // Stimulating Match
    if (
      (e1 === "earth" && e2 === "water") ||
      (e1 === "water" && e2 === "earth")
    )
      return 82; // Grounded Match
  ...
  function sortProfilesByCompatibility(userSign) {
    ...
    profiles.sort(
      (a, b) =>
        getCompatibility(userSign, b.sign) - getCompatibility(userSign, a.sign),
    );
  }
